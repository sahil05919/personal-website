...
/*
  Extracted from app/experience/page.tsx so it can read the reader's language choice.
...
function buildGlance(
  head: ExperienceEntry,
  rest: readonly ExperienceEntry[],
  prologueNote: string,
): GlanceItem[] {
  return [
    { id: head.id, marker: "Since 2011", label: head.title, note: prologueNote },
    ...rest.map((entry) => ({
      id: entry.id,
      marker: entry.year ?? "",
      label: entry.title,
...
export default function ExperienceChapter() {
  /* Hinglish. Same ids and order in both files, so anchors resolve either way.
     The glance list has to be derived INSIDE the component rather than at module
     scope: built from the English arrays once, it kept twelve English chapter
     titles in the rail while the chapter beside it read in Hinglish, which was
     the one place the two readings visibly disagreed. */
  const label = useVariant(chapterLabel, chapterLabelHi);
  const lede = useVariant(standfirst, standfirstHi);
  const rows = useVariant(ledger, ledgerHi);
  const turn = useVariant(hinge, hingeHi);
  const opening = useVariant(prologue, prologueHi);
  const chapters = useVariant(entries, entriesHi);
  const closing = useVariant(coda, codaHi);
  const glance = useVariant(
    { heading: "At a glance", summary: "Fifteen years, mostly not chosen.", note: "Fifteen years, mostly not chosen. Jump to any of it.", prologue: "Prologue" },
    experienceGlanceHi,
  );
  const glanceItems = buildGlance(opening, chapters, glance.prologue);
      ...
      */}
      <div className="mx-auto max-w-[76rem] px-6 pb-[100px] sm:pb-40 lg:px-10">
        ...
        <Reveal as="div" duration={0.7} margin="-60px" className="mt-24 sm:mt-40">
          <Entry entry={opening} isPrologue />
        </Reveal>
